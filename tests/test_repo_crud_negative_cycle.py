...
class TestNegativeCRUD:
    ...
    @allure.description('Проверяем, что нельзя создать репозиторий с именем, которое уже используется в другом репозитории')
    ...
    @allure.description('Проверка, что при запросе на просмотр несуществующего репозитория отдаётся ошибка')
    ...
    @allure.description('Проверка, что при запросе на удаление несуществующего репозитория отдаётся ошибка')
    @allure.severity(allure.severity_level.NORMAL)
    ...
    @allure.description('Проверка, что неавторизованному юзеру нельзя просмотреть профиль любого юзера')
    @allure.severity(allure.severity_level.CRITICAL)
    ...
    @allure.description('Проверка, что нельзя создать репозиторий не заполнив обязательные поля')
    @allure.severity(allure.severity_level.NORMAL)
    ...
    @allure.description('Проверка, что нельзя создать репозиторий без тела запроса вообще')
    @allure.severity(allure.severity_level.NORMAL)
    ...
    @allure.description('Проверка, что нельзя просмотреть профиль какого угодно юзера с невалидным токеном')
    @allure.severity(allure.severity_level.CRITICAL)
